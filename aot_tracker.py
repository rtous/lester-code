...
class AOTTracker(object):
    def __init__(self, cfg, device, gpu_id=0):
        self.gpu_id = gpu_id
        if device=='gpu':
    ...
    def add_reference_frame(self, frame, mask, obj_nums, frame_step, incremental=False):
        ...
        if self.gpu_id=='cpu':#RUBEN
            frame = sample[0]['current_img'].unsqueeze(0).float()
        else:
            frame = sample[0]['current_img'].unsqueeze(0).float().cuda(self.gpu_id)
        #frame = sample[0]['current_img'].unsqueeze(0).float().cuda(self.gpu_id)
        if self.gpu_id=='cpu':
            mask = sample[0]['current_label'].unsqueeze(0).float()
        else:
    ...
    def track(self, image):
        ...
        sample = self.transform(sample)
        if self.gpu_id=='cpu':
            image = sample[0]['current_img'].unsqueeze(0).float()
        else:
...
def get_aot(args):
    ...
    # init AOTTracker
    if (args["device"]=="gpu"):#RUBEN
        tracker = AOTTracker(cfg, args["device"], args['gpu_id'])
    else:
        tracker = AOTTracker(cfg, args["device"], 'cpu')
