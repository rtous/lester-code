...
def main(scene, keyword_lists):
	segment_scene(scene, keyword_lists)

def segment_scene(scene, keyword_lists):
	...
	for i, keyword_list in enumerate(keyword_lists):
		...
		Seg_Tracker, _, _, _ = init_SegTracker(aot_model, long_term_mem, max_len_long_term, sam_gap, max_obj_num, points_per_side, origin_frame, DEVICE)
		...
		predicted_mask, annotated_frame= Seg_Tracker.detect_and_seg_v2(origin_frame, grounding_caption, box_threshold, text_threshold)
...
def init_SegTracker(aot_model, long_term_mem, max_len_long_term, sam_gap, max_obj_num, points_per_side, origin_frame, DEVICE):
    ...
    #RUBEN
    sam_args["device"] = DEVICE
    aot_args["device"] = DEVICE
...
def SegTracker_add_first_frame(Seg_Tracker, origin_frame, predicted_mask):
    ...
    with torch.cuda.amp.autocast():
    #with torch.cpu.amp.autocast(device_type="cpu", dtype=torch.bfloat16):
